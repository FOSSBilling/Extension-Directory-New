...
class BadgeBuilder
{
    ...
    public function buildABadge(string $id, string $type, ?string $color = null): string
    {
        $key = 'badge_' . hash('xxh3', serialize([$id, $type, $color]));

        return $this->cache->get($key, function (ItemInterface $item) use ($id, $type, $color): string {
            $item->expiresAfter(3600); // Cached for an hour just to reduce load on shields.io

            $extension = $this->extensionManager->getExtensionInfo($id, false);
            ...
            switch (strtolower($type)) {
                ...
                case 'repo_type_1':
                    ...
                    $logo = $this->tools->getRepoType($repoUrl);
                    $badgeUrl = "https://shields.io/badge/Repository-Source_Code-{$color}?logo={$logo}";
                    break;
                case 'repo_type_2':
                    $repoUrl = $extension['repo'];
                    $logo = $this->tools->getRepoType($repoUrl);
                    $repo = str_replace('-', '--', $this->tools->getRepoName($repoUrl));
                    $badgeUrl = "https://shields.io/badge/Repository-$repo-{$color}?logo={$logo}";
                    break;
                case 'author':
                    $author = $extension['author']['name'];
                    $author = str_replace('-', '--', $this->tools->getRepoName($author));
                    $author = str_replace('_', '__', $this->tools->getRepoName($author));
                    // Shields.io allows custom logos if we base it base64 encoded, so we do so here
                    $logo   = 'data:image/svg%2bxml;base64,' . base64_encode(self::authorIcon);
            ...
            // Attempt to fetch the badge with error handling
            try {
                $response = $this->httpClient->request('GET', $badgeUrl);
                ...
                // Reduced cache time on error so it can self-resolve
                $item->expiresAfter(300);
                // Return empty SVG placeholder on error
                return '<svg xmlns="http://www.w3.org/2000/svg" width="90" height="20"><text x="5" y="15" fill="#999">Error</text></svg>';
            }
        });
    }
}
